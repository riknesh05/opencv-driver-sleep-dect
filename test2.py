...
# Points to highlight (upper and lower lids)
HIGHLIGHT_POINTS = {
    "left": [385, 380],
    "right": [160, 144]
}
...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        ...
        # Highlight 2 specific points (upper & lower eyelids)
        for eye, points in HIGHLIGHT_POINTS.items():
            for i in points:
                x = int(landmarks[i].x * w)
                y = int(landmarks[i].y * h)
                cv2.circle(frame, (x, y), 4, (255, 0, 0), -1)  # Blue dots for focus points
