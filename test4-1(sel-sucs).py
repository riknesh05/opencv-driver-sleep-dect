...
def play_alert(alert_type):
    alert_sounds = {
        "SLEEPING": r"",  # audio file location
    }
...
while True:
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            # Reset counts on active state detection
            if left_blink == 2 and right_blink == 2:  # Both eyes fully open
                active_count += 1
                sleep_count = drowsy_count = 0
                if active_count > 10:  # Increase threshold for active detection
                    current_status = "ACTIVE :)"
                    status_color = (0, 255, 0)

            elif left_blink == 1 or right_blink == 1:  # Either eye semi-open (drowsy)
                drowsy_count += 1
            ...
            elif left_blink == 0 or right_blink == 0:  # Eyes closed (sleeping)
                sleep_count += 1
